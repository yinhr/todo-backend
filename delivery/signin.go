...
type (
  SigninHandler struct {
    SigninUsecase signin.Usecase
  }
)

func (h *SigninHandler) Get(c echo.Context) error {
  return c.JSON(http.StatusOK, nil)
}

func (h *SigninHandler) Post(c echo.Context) error {
  u := new(model.User)
  if err := c.Bind(u); err != nil {
    log.Error(err)
    return c.JSON(http.StatusBadRequest, util.NewBadRequestError("400: BadRequest"))
  }
  err := h.SigninUsecase.Create(util.GetContext(c), u)
  if err != nil {
...
func (h *SigninHandler) Delete(c echo.Context) error {
  if err := session.Delete(session.Name, c); err != nil {
